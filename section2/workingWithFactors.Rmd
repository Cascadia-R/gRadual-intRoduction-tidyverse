...
pets <- data.frame(id=c('C1','C2','D1','D4','D8', 'C4', 'D5', 'C10'),
                names=factor(c("Morris", "Morris", "Fido", "Mr Bowser", "Fido", "Lady Sheba", "Hubert", "Apples")),
                weights=c(6,7,15,20,40, 10, 60, 17),
                animal=factor(c("cat", "cat", "dog", "dog", "dog", "cat", "dog", "cat")),
                ##because it's an ordered category, we have to specify 
                ##order as the levels argument
                ageCategory= ordered(c("0-2", "5-8", "5-8", "3-4", "3-4", "5-8", "0-2", "5-8"), 
                                       levels=c("0-2", "3-4", "5-8")),
                shotsCurrent=c("No", "Yes", NA, "No", "Yes", "No", "No", "Yes"))
...
Let's plot the weights for each pet.

```{r}
ggplot(data=pets, aes(x=id,y=weights)) + geom_bar(stat="identity")
```
...
Instead of stacking, we can also dodge the bars.
...
```{r}
ggplot(pets, aes(x=shotsCurrent, y=weights)) + geom_boxplot()
...
```{r}
ggplot(pets, aes(x=shotsCurrent, y=weights)) + geom_violin()
